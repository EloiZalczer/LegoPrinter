...
#ifdef CHOCO
int chocolate_mode = 0; //default : chocolate printed is disabled

void lcd_set_choco(){ //Changes chocolate_mode when called
  if (chocolate_mode)
    {chocolate_mode = 0; SERIAL_ECHOLNPGM("chocolate_mode=0\n");}
  else {chocolate_mode = 1; SERIAL_ECHOLNPGM("chocolate_mode=1\n");};
}
#endif
...
static void lcd_prepare_menu() {
  ...
  #ifdef CHOCO
    if (chocolate_mode) 
    MENU_ITEM(function, MSG_DIS_CHOCO, lcd_set_choco);
    else MENU_ITEM(function, MSG_EN_CHOCO, lcd_set_choco);
  #endif
